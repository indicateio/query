...
func TestSelectComplex(t *testing.T) {
    expectedSql :=
        "SELECT ta.uid, ta.given_name, ta.family_name, CONCAT(ta.given_name, ' ,', ta.family_name) AS formatted_name, ta.primary_email, ta.primary_email FROM table_name_a AS ta INNER JOIN table_name_b AS tb ON ta.uid = tb.table_a_uid AND tb.parent_uid <> ta.uid LEFT JOIN OUTER table_name_c AS tc ON tb.uid = tc.table_b_uid INNER JOIN (SELECT tc.uid FROM table_name_a AS tc WHERE tc.created_date_time > ? LIMIT 10) AS filter ON filter.uid = ta.uid WHERE da.account_uid = ? ORDER BY ta.given_name, ta.family_name DESC, ta.display_name, ta.formatted_name ASC GROUP BY ta.uid LIMIT ? OFFSET ? UNION SELECT tc.uid FROM table_name_a AS tc WHERE tc.created_date_time > ? LIMIT 10"
